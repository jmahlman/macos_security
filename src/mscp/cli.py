...
class Customparser(argparse.ArgumentParser):
    ...
    def error(self, message: str) -> None:
        """Print help on missing-required-arg failures; log others via loguru.

        When the failure is `the following arguments are required: ...`
        (i.e. the user ran the command with no args), just print help and
        exit — that's what they wanted. Anything else (unrecognized flag,
        invalid value, etc.) is a real error, so log it before help.
...
def parse_cli() -> None:
    ...
    def _os_version_completer(prefix, parsed_args, **kwargs):
        platforms = mscp_data.get("versions", {}).get("platforms", {})
        os_name = getattr(parsed_args, "os_name", None) or "macos"
        entries = platforms.get(os_name, [])
        return [str(e["os_version"]) for e in entries]

    os_version_action = parser.add_argument(
        "--os_version",
        default=get_macos_version(),
        type=float,
        help="version of the operating system to be referenced when generating guidance (eg: 14.0, 15.0).",
    )
    os_version_action.completer = _os_version_completer
    ...
    def _baseline_yaml_completer(prefix, parsed_args, **kwargs):
        candidates: list[str] = []
        seen: set[str] = set()
        for base in (config.get("baseline_dir"), config["custom"].get("baseline_dir")):
            if not base:
                continue
            root = Path(base)
            if not root.is_dir():
    ...
    baseline_positional = guidance_parser.add_argument(
        "baseline",
        default=None,
...
baseline files are generated by the `mscp.py baseline` command""",
    ...
    baseline_positional.completer = _baseline_yaml_completer
    ...
    logo_action = guidance_parser.add_argument(
        "-l",
        "--logo",
        default=None,
    ...
    logo_action.completer = argcomplete.FilesCompleter(("png", "jpg", "jpeg", "svg"))
    guidance_parser.add_argument(
        "-L",
        "--language",
        default="en",
...
compliance script (e.g. disa_stig, cis.benchmark)
    ...
    mapping_csv_action = mapping_parser.add_argument(
        "-c",
        "--csv",
        default=None,
        required=True,
    ...
    mapping_csv_action.completer = argcomplete.FilesCompleter(("csv",))
